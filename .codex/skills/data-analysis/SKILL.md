...
- `.codex/rules/r-code-conventions.md` — [one-line summary of most relevant rule]
- `.codex/rules/content-invariants.md` — [INV-9, INV-10, INV-11, INV-12 applicable]
...
For regressions, simulations, or bootstrap loops that take more than a couple of minutes, launch via Bash with `run_in_background: true` and then use OpenAI's **Monitor tool** to stream R stdout into the conversation in real time. Pattern:
...
This avoids the polling-loop anti-pattern (`sleep 30; check; sleep 30; check`) and avoids burning cache on idle waits. Especially useful when paired with the [Cost-Conscious Parallelism](https://psantanna.com/codex-academic-workflow/workflow-guide.html#cost-conscious-parallelism) section of the guide.
