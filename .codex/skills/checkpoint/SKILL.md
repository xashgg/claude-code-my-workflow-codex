...
- Before a context reset or handoff would otherwise discard mid-plan context.
...
- For the narrative *what happened* — that lives in `quality_reports/session_logs/` (see `.codex/rules/session-logging.md`).
...
- `.codex/skills/checkpoint/SKILL.md:42` — body draft, needs trigger-phrase tightening
...
If the user says "yes" / "all" / "1 and 3" — append to MEMORY.md (root, the committed one) using the `[LEARN]` format. If the candidate is machine-specific (paths, tool versions, personal preference), recommend the user route it to `.codex/state/personal-memory.md` instead per `.codex/rules/meta-governance.md`.
...
✓ Checkpoint saved: quality_reports/checkpoints/YYYY-MM-DD_<slug>.md
  ...
  Resume prompt: paste the checkpoint resume prompt into a fresh Codex session   (or paste the file's "Resume prompt" into a fresh session)
```
...
- `.codex/rules/session-logging.md` — narrative companion. **Do not duplicate** — the checkpoint references the latest session log by path; it does not re-tell the session story.
- `.codex/rules/plan-first-workflow.md` — checkpoint reads the active plan; if no plan exists, recommend the user enter plan mode before invoking `/checkpoint`.
...
**Result:** Next session: `a fresh Codex session`, then `read quality_reports/checkpoints/2026-04-27_v180-polisci.md and start at action 1`.
