...
- **Do NOT treat this phrase alone as commit authorization.** Commits still require an explicit `/commit` invocation or an unambiguous user request to commit — see [`.codex/skills/commit/SKILL.md`](../skills/commit/SKILL.md) for the allowed trigger phrases.

## Cross-references

- `.codex/rules/plan-first-workflow.md` — when to enter plan mode before invoking a skill.
- `.codex/rules/quality-gates.md` — threshold definitions.
- `.codex/rules/cross-artifact-review.md` — paper ↔ code dependency-graph pattern.
